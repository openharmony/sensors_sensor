...
void SensorService::ReportSensorSysEvent(uint32_t sensorId, bool enable, int32_t pid)
{
    ...
    std::string message;
    if (enable) {
        HiSysEvent::Write(HiSysEvent::Domain::SENSORS_SENSOR, "ENABLE_SENSOR", HiSysEvent::EventType::STATISTIC,
            "LEVEL", logLevel, "UID", uid, "PKG_NAME", packageName, "TYPE", sensorId);
    } else {
        HiSysEvent::Write(HiSysEvent::Domain::SENSORS_SENSOR, "DISABLE_SENSOR", HiSysEvent::EventType::STATISTIC,
            "LEVEL", logLevel, "UID", uid, "PKG_NAME", packageName, "TYPE", sensorId);
    }
}
