...
| Cangjie 绑定 | `frameworks/cj/` | Cangjie 语言传感器 FFI 绑定 | `src/` |
| Taihe/ETS 绑定 | `frameworks/ets/taihe/` | Taihe 声明式传感器接口 | - |
| Sensor Service | `services/` | SA 3601 服务端，传感器注册/连接/分发/电源/数据通道 | `src/sensor_service.cpp` |
| HDI Connection | `services/hdi_connection/` | HDI 驱动连接/重连/容错，V3.0 和 V1.0 双适配 | `interface/src/sensor_hdi_connection.cpp` |
| HDI 硬件实现 | `services/hdi_connection/hardware/` | ISensorInterface 调用实现 + ENG Mock | `include/hdi_service_impl.h` |
| HDI 兼容适配 | `services/hdi_connection/adapter/` | V1.0 兼容连接（CompatibleConnection） | `include/compatible_connection.h` |
...
| Vibration Convert | `vibration_convert/core/` | WAV 解析 + STFT/FFT/MFCC + 峰值检测 + 触觉事件生成 | `core/algorithm/` |
