        ...
        namespace
        {
            ...
            FuzzSession InitSession(FuzzedDataProvider &fdp)
            {
                ...
                session.sensorMode = fdp.ConsumeIntegralInRange<int32_t>(
                    static_cast<int32_t>(SENSOR_DEFAULT_MODE),
                    static_cast<int32_t>(SENSOR_FIFO_MODE));

                session.pid = fdp.ConsumeIntegralInRange<int32_t>(MIN_PID, MAX_PID);

                session.samplingInterval = fdp.ConsumeIntegralInRange<int64_t>(
                    MIN_SAMPLING_INTERVAL, MAX_SAMPLING_INTERVAL);

                session.reportInterval = fdp.ConsumeIntegralInRange<int64_t>(
                    MIN_SAMPLING_INTERVAL, MAX_SAMPLING_INTERVAL);
                ...
                errno_t ret = memcpy_s(session.sensorUser.name, NAME_MAX_LEN,
                                       userName.c_str(), copyLen);
                if (ret != EOK)
                {
                    (void)strncpy_s(session.sensorUser.name, NAME_MAX_LEN, "SensorFuzzTest",
                                    NAME_MAX_LEN - 1);
                }
                else
                {
            ...
            // 动作3: 取消订阅传感器
            static void HandleUnsubscribeSensor(FuzzSession &session,
            ...
            // 动作5: 停用传感器
            static void HandleDeactivateSensor(FuzzSession &session,
                                               FuzzedDataProvider &fdp)
            {
            ...
            static void HandleSetBatch(FuzzSession &session, FuzzedDataProvider &fdp)
            {
                ...
                {
                    // 重新生成间隔值
                    int64_t newSampling = fdp.ConsumeIntegralInRange<int64_t>(
                        MIN_SAMPLING_INTERVAL, MAX_SAMPLING_INTERVAL);
                    int64_t newReport = fdp.ConsumeIntegralInRange<int64_t>(
                        MIN_SAMPLING_INTERVAL, MAX_SAMPLING_INTERVAL);
                    ...
                    (void)SetBatch(session.sensorTypeId, &session.sensorUser, newSampling,
                                   newReport);
                }
            }
            ...
            // 动作8: 获取激活传感器信息
            static void HandleGetActiveSensorInfos(FuzzSession &session,
                                                   FuzzedDataProvider &fdp)
            {
            ...
            // 动作12: 注册活跃信息回调
            static void HandleRegisterCallback(FuzzSession &session,
                                               FuzzedDataProvider &fdp)
            {
            ...
            // 动作13: 取消注册回调
            static void HandleUnregisterCallback(FuzzSession &session,
                                                 FuzzedDataProvider &fdp)
            {
            ...
            // ======= 执行单个动作 =======
            static void ExecuteAction(ActionType action, FuzzSession &session,
                                      FuzzedDataProvider &fdp)
            {
                switch (action)
                {
            ...
            // ======= 执行操作序列 =======
            static void ExecuteOperationSequence(FuzzSession &session,
                                                 FuzzedDataProvider &fdp, size_t ops)
            {
                for (size_t i = 0; i < ops && fdp.remaining_bytes() > 0; i++)
                {
                    uint8_t actionValue = fdp.ConsumeIntegral<uint8_t>() %
                                          static_cast<uint8_t>(ActionType::ACTION_MAX);
                    ...
                    {
                        int32_t sleepMs =
                            fdp.ConsumeIntegralInRange<int32_t>(MIN_SLEEP_MS, MAX_SLEEP_MS);
