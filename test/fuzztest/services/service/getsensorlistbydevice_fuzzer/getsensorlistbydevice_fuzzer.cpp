...
#include "getsensorlistbydevice_fuzzer.h"
...
bool GetSensorListByDeviceFuzzTest(const uint8_t *data, size_t size)
{
    SetUpTestCase();
    int32_t deviceId = 0;
    size_t consumed = GetObject<int32_t>(deviceId, data, size);
    if (consumed == 0) {
        return false;
    }
    g_service->GetSensorListByDevice(deviceId);
    return true;
}
} // namespace Sensors
} // namespace OHOS
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::Sensors::GetSensorListByDeviceFuzzTest(data, size);
    return 0;
}
