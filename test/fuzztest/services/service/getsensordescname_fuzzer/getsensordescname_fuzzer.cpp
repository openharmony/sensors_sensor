...
#include "getsensordescname_fuzzer.h"
...
bool GetSensorDescNameFuzzTest(const uint8_t *data, size_t size)
{
    SetUpTestCase();
    SensorDescription sensorDesc;
    size_t consumed = GetObject<SensorDescription>(sensorDesc, data, size);
    if (consumed == 0) {
        return false;
    }
    std::string sensorDescName;
    g_service->GetSensorDescName(sensorDesc, sensorDescName);
    return true;
}
} // namespace Sensors
} // namespace OHOS
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::Sensors::GetSensorDescNameFuzzTest(data, size);
    return 0;
}
