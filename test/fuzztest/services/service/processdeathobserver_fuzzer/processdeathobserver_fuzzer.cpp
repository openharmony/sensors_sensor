...
#include "processdeathobserver_fuzzer.h"
...
bool ProcessDeathObserverFuzzTest(const uint8_t *data, size_t size)
{
    SetUpTestCase();
    if (g_remote == nullptr) {
        return false;
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::Sensors::ProcessDeathObserverFuzzTest(data, size);
    return 0;
}
