...
#include "registerclientdeathrecipient_fuzzer.h"
...
bool RegisterClientDeathRecipientFuzzTest(const uint8_t *data, size_t size)
{
    SetUpTestCase();
    if (g_remote == nullptr) {
        return false;
    ...
    g_service->RegisterClientDeathRecipient(g_remote, pid);
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::Sensors::RegisterClientDeathRecipientFuzzTest(data, size);
    return 0;
}
