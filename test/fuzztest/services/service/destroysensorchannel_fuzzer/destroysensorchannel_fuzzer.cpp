...
#include "destroysensorchannel_fuzzer.h"
...
bool DestroySensorChannelFuzzTest(const uint8_t *data, size_t size)
{
    SetUpTestCase();
    if (g_remote == nullptr) {
        return false;
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::Sensors::DestroySensorChannelFuzzTest(data, size);
    return 0;
}
