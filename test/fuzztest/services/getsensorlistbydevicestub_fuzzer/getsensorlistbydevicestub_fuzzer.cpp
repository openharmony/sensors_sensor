...
#include "getsensorlistbydevicestub_fuzzer.h"
...
void SetUpTestCase()
{
    ...
    TokenInfoParams infoInstance = {
        ...
        .perms = perms,
        .acls = nullptr,
        .processName = "GetSensorListByDeviceStubFuzzTest",
        .aplStr = "system_core",
    };
...
bool OnRemoteRequestFuzzTest(const uint8_t *data, size_t size)
{
    ...
    int32_t deviceId = 0;
    if (size < sizeof(deviceId)) {
        return false;
    }
    errno_t ret = memcpy_s(&deviceId, sizeof(deviceId), data, sizeof(deviceId));
    if (ret != EOK) {
        return false;
    }
    datas.WriteInt32(deviceId);
    datas.RewindRead(0);
    MessageParcel reply;
    MessageOption option;
    g_service->OnRemoteRequest(static_cast<uint32_t>(ISensorServiceIpcCode::COMMAND_GET_SENSOR_LIST_BY_DEVICE),
        datas, reply, option);
    return true;
}
} // namespace Sensors
